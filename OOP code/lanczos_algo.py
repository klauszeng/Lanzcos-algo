...
import numpy as np
from  MatvecLammps import MatvecLammps
from atomNum import atomNum



def lanczos_algo(filename):

    N = atomNum(filename)
    print(N)
    vkml = np.random.rand(N*3,1)
    vkml = np.divide(vkml, np.linalg.norm(vkml))
    vk = MatvecLammps(vkml, filename)
